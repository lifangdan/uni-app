<template>
	<view class="like-button">
		...
		<list class="animate-wrap">
			<cell class="a-img" v-for="(item,index) in viewList" :key="item.elId" :ref="item.elId" :style="{
		...
		<view class="animate-wrap">
			<view class="a-img" v-for="(item,index) in viewList" :key="item.elId" :ref="item.elId" :style="{
            'right': site.x || site[0] + 'rpx',
            'bottom': site.y || site[1] + 'rpx'
          }">
				<image :style="{
		...
		methods: {
			handleClick(e) {
				...
				setTimeout(() => {
					this.$nextTick(() => {
						let _n = 1
						if (this.large) _n = typeof(this.large) === 'number' ? this.large : 1.5;
						// #ifndef APP-NVUE
						_item.animation.translateY(-_dirY).translateX(_dirX).scale(_n, _n).opacity(0)
						.step()
